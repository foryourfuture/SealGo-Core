...
export default {
  handle: (ctx: SealGo): void => {
    ...
    cmd.program
      .command('use')
      .arguments('[module]')
      .description('use modules of sealgo')
        ...
        try {
          ...
          const config = {
            ...
            plugins: {
              ...
              default: Object.keys(ctx.config.sealgoPlugins).filter((item: string) => ctx.config.sealgoPlugins[item])
            }
          }
          ...
          if (answer['plugins']) {
            let plugins = ctx.getConfig('sealgoPlugins')
