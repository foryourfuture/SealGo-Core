...
const run = (ctx: SealGo, options: Options): void => {
  ...
  if (options.offline) { // offline mode
    ...
    options.template = !options.hasSlash
      ? 'SealGo/sealgo-template-' + options.template // official template
      : options.template
 ...
 */
const downloadAndGenerate = (ctx: SealGo, options: Options): void => {
...
export default {
  handle: (ctx: SealGo): void => {
    ...
    cmd.program
      ...
      .description('create sealgo plugin\'s development templates')
      .action(async (template: string, project: string, program: any) => {
        ...
        try {
          ...
          const tmp = path.join(homedir(), '.sealgo/templates', template.replace(/[\/:]/g, '-')) // for caching template
      ...
      .on('--help', () => {
        ...
        console.log('  $ sealgo init plugin my-project')
        console.log()
        console.log(chalk.gray('  # create a new project straight from a github template'))
        console.log('  $ sealgo init username/repo my-project')
        console.log()
      })
  }
}
