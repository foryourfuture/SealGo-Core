...
// handle modules config -> save to sealgo config file
const handleConfig = async (ctx: SealGo, prompts: PluginConfig, module: string, name: string): Promise<void> => {
...
export default {
  handle: (ctx: SealGo): void => {
    ...
    cmd.program
      ...
      .description('configure config of sealgo modules')
      .action(async (module: string, name: string) => {
        try {
          ...
          switch (module) {
            ...
            case 'plugin':
              if (name) {
                if (!name.includes('sealgo-plugin-')) {
                  name = `sealgo-plugin-${name}`
                }
                if (Object.keys(ctx.config.sealgoPlugins).includes(name)) {
