...
// Thanks to vs-sealgo: https://github.com/Spades-S/vs-sealgo/blob/master/src/extension.ts
const getClipboardImage = (ctx: SealGo): Promise<ClipboardImage> => {
  ...
  return new Promise<ClipboardImage>((resolve: Function): void => {
    ...
    let execution = null
    // for SealGo GUI
    let env = ctx.config.SEALGO_ENV === 'GUI'
    const platformPaths: {
      [index: string]: string
    } = {
      ...
      if (platform === 'linux') {
        if (data.toString().trim() === 'no xclip') {
          return ctx.emit('notification', {
            ...
            body: 'Please install xclip before run sealgo'
          })
        }
      }
